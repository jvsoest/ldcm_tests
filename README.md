...
The test data is downloaded from an XNAT server using `download.py`:
...
**What it does:**
- Connects to XNAT server at `https://xnat.health-ri.nl`
...
After downloading the DICOM files, convert them to LinkedDicom TTL format using the LinkedDicom tool (not included in this repository).
...
- Outputs results to `data_analysis_results_rdf4j.csv`
...
### 2. RDFLib Per-Patient Tests (`analyse.py`)
...
**Outputs:**
- `data_analysis_times.csv`: Per-patient loading and querying times
- `data_analysis_results.csv`: Aggregated query results

### 3. RDFLib Batch Loading Tests (`analyse_once.py`)
...
**Outputs:**
- `data_analysis_results_once.csv`: Query results for batch-loaded data
- Console output with timing breakdowns

### 4. SPARQL Query (`query.sparql`)

The test query links radiotherapy structure sets with CT imaging series:
...
**Query Complexity:**
- Multiple nested sequence items
- Optional fields for CT series metadata
- Complex joins across patient → study → series → images
...
The statistical analysis script (`statistical_analysis.py`) performs correlation analysis between:

1. **Number of Triples vs File Size**
   - Correlation coefficient calculation
   - Scatter plot: `file_size_vs_number_of_triples.png`

2. **Number of Triples vs Processing Time**
   - Correlation between graph size and query execution time
   - Scatter plot: `elapsed_time_vs_number_of_triples.png`

3. **File Size vs Processing Time**
   - Correlation between file size and elapsed time
   - Scatter plot: `elapsed_time_vs_file_size.png`

### Data Preparation

The analysis:
- Merges `data_analysis_times.csv` with `data_triples_analysis.csv`
...
| File | Description |
|------|-------------|
| `data_analysis_results_rdf4j.csv` | Query results from RDF4J test |
| `data_analysis_results.csv` | Query results from per-patient RDFLib test |
| `data_analysis_results_once.csv` | Query results from batch RDFLib test |
| `data_analysis_times.csv` | Per-patient processing times |
| `data_triples_analysis.csv` | Triple counts and file sizes per patient |
| `file_size_vs_number_of_triples.png` | Correlation plot |
| `elapsed_time_vs_number_of_triples.png` | Performance correlation plot |
| `elapsed_time_vs_file_size.png` | Size/time correlation plot |
